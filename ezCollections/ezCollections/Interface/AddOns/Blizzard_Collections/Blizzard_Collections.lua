...
function CollectionsJournal_SetTab(self, tab)
	...
	ezCollections:SetCVar("petJournalTab", tab);
...
local function ShouldShowHeirloomTabHelpTip()
	if ezCollections:GetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_HEIRLOOM_JOURNAL_TAB) or IsKioskModeEnabled() then
		return false;
	end
...
local function ShouldShowWardrobeTabHelpTip()
	if ezCollections:GetCVarBitfield("closedInfoFrames", LE_FRAME_TUTORIAL_TRANSMOG_JOURNAL_TAB) or IsKioskModeEnabled() then
		return false;
	end
